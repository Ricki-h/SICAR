<script setup>
    ...
    function handleImages(event) {
        ...
        if (files.length + images.value.length > 3) {
            alert('Só é possível anexar até 3 imagens')
            event.target.value = ''
            return
        }
...
<template>
    ...
    <main class="ui-container-xl px-6 sm:px-12 lg:px-28 flex-col flex gap-6 mx-auto mb-25">
        ...
        <form class="grid grid-cols-1 sm:grid-cols-2 gap-4" @submit.prevent="submitDenuncia">
            <div class="flex flex-col gap-2">
                <label class="font-bold text-title">Título</label>
            ...
            <div class="flex flex-col gap-2">
                <label class="font-bold text-title">Endereço</label>
            ...
            <div class="flex flex-col gap-2 sm:order-2">
                <label class="font-bold text-title">Tipo</label>
                ...
                <div>
                    <label class="font-bold text-title">Foto</label>
                    <label class="border border-dashed flex flex-col items-center justify-center sm:order-1 p-6 border-clarinho bg-bg text-text placeholder:text-clarinho focus:outline-orange-600 hover:cursor-pointer mt-2">
                        <input type="file" accept="image/*" multiple class="hidden" @change="handleImages">
                        <svg xmlns="http://www.w3.org/2000/svg" width="100" height="79" viewBox="0 0 100 79" fill="none" stroke="currentColor" class="stroke-clarinho">
                            <path d="M7.62012 2H91.4404C94.5443 2 97.0605 4.51625 97.0605 7.62012V71.1201C97.0605 74.224 94.5443 76.7402 91.4404 76.7402H7.62012C4.51625 76.7402 2 74.224 2 71.1201V7.62012L2.00684 7.33105C2.15724 4.36148 4.61317 2 7.62012 2ZM54.2861 32.7852L43.125 47.374L38.3389 41.1182L36.75 39.042L35.1611 41.1182L22.4111 57.7852L19.9521 61H79.0479L76.5889 57.7852L57.4639 32.7852L55.875 30.708L54.2861 32.7852ZM70.9521 57H28.0479L36.75 45.625L41.5361 51.8818L43.125 53.958L44.7139 51.8818L55.875 37.291L70.9521 57ZM27 24.5C27 25.8807 25.8807 27 24.5 27C23.1193 27 22 25.8807 22 24.5C22 23.1193 23.1193 22 24.5 22C25.8807 22 27 23.1193 27 24.5ZM31 24.5C31 20.9101 28.0899 18 24.5 18C20.9101 18 18 20.9102 18 24.5C18 28.0898 20.9101 31 24.5 31C28.0899 31 31 28.0899 31 24.5Z"stroke-width="4"/>
                        </svg>
                    </label>
                </div>
            <BaseButton type="submit" color="blue" :disabled="loading" class="w-full sm:order-3 h-fit">{{ loading ? 'Enviando...' : 'ENVIAR DENÚNCIA' }}</BaseButton>
        </form>
        <div v-if="previews.length" class="flex justify-evenly gap-2 flex-wrap">
            <div v-for="(img, index) in previews" :key="index" class="relative w-36 h-36 overflow-hidden bg-bg-blue p-2">
                <img :src="img" class="h-full w-full object-cover">
                <button type="button" @click="removeImage(index)" class="hover:cursor-pointer absolute top-1 right-1 rounded-full text-white bg-red-600 w-5 h-5 flex items-center justify-center"> x </button>
            </div>
        </div>
    </main>
